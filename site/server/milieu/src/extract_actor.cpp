///////////////////////////////////////////
// This file is part of Kepler's Horizon //
//                                       //
// Licensed under BSD 3-Clause License   //
//                                       //
// Copyright (c) 2025, sibomots          //
///////////////////////////////////////////
#include "extract_actor.h"

#include <sstream>

#include "db.h"
#include "extract_modes.h"
#include "extract_strategy.h"
#include "logger.h"
#include "statemachine.h"
#include "telemetry.h"

bool ExtractActor::invoke(void)
{
    bool bres = false;
    GameState s = StateMachine::instance().get_game_state();
    char owner = StateMachine::instance().get_current_player();

    // Check inhibits
    std::string inhibit_error;
    if (!StateMachine::instance().check_inhibits(CommandID::EXTRACT,
                                                 inhibit_error))
    {
        Telemetry::instance().write("Error: " + inhibit_error);
        bres = false;
    }
    else
    {
        if (get_scan_mode())
        {
            ExtractParam ep = ExtractParam::Builder()
                                  .set_game_id(s.game_id)
                                  .set_player(owner)
                                  .set_scan_mode()
                                  .build();
            MilieuAgentParam mp(ep);
            bres = MilieuAgent::instance().apply(mp);
        }
        else if (get_extract_mode())
        {
            ExtractParam ep = ExtractParam::Builder()
                                  .set_game_id(s.game_id)
                                  .set_player(owner)
                                  .set_extract_mode()
                                  .set_resource(m_resource)
                                  .set_ship_code(m_ship_code)
                                  .build();
            MilieuAgentParam mp(ep);
            bres = MilieuAgent::instance().apply(mp);
        }
    }
    return bres;
}
