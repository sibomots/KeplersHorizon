...
bool DeployCommand::invoke(void)
{
    ...
    std::string player_side =
        (KH_EQU(active_player, 'A')) ? s.home_side_A : s.home_side_B;

    // BIGBUG  We need a query that can get BOTH territories
    // announced when the first base is claimed.

    if (player_side.empty())
    {
        // First deployment - set player's side
        if (KH_EQU(active_player, 'A'))
        {
            s.home_side_A = base_side;
            s.home_side_B = (KH_EQU(base_side, "A")) ? "B" : "A";
        }
        else
        {
            s.home_side_B = base_side;
            s.home_side_A = (KH_EQU(base_side, "A")) ? "B" : "A";
        }
        ...
        std::string sideA = s.home_side_A;
        std::string sideB = s.home_side_B;
        if (!sideA.empty() && !sideB.empty())
        {
            // Check if base_stars already populated for this game
            auto existing = db.Query(
                "SELECT 1 FROM base_stars WHERE game_id=? LIMIT 1",
                {m_game_id});
            if (existing.empty())
            {
                // Insert all base hexes for player A (side A's bases)
                auto basesA = db.Query(
                    "SELECT hex_id FROM star_systems "
                    "WHERE module_id=? AND is_base=1 AND base_side=?",
                    {mod_id, sideA});
                for (const auto& row : basesA)
                {
                    if (!row.empty())
                    {
                        db.Exec(
                            "INSERT INTO base_stars (game_id, hex_id, owner) "
                            "VALUES (?, ?, 'A')",
                            {m_game_id, row[0]});
                    }
                }
                // Insert all base hexes for player B (side B's bases)
                auto basesB = db.Query(
                    "SELECT hex_id FROM star_systems "
                    "WHERE module_id=? AND is_base=1 AND base_side=?",
                    {mod_id, sideB});
                for (const auto& row : basesB)
                {
                    if (!row.empty())
                    {
                        db.Exec(
                ...
                Logger::instance().info(
                    std::format("[DEPLOY] Populated base_stars for game {}:"
                                " A={} bases, B={} bases",
...
bool MoveCommand::invoke(void)
{
    ...
    std::unordered_map<std::string, std::pair<std::string, bool>> starByHex;
    {
        ...
        for (const auto& row : sysList)
        {
            if (row.size() >= 3)
            {
                ...
                const bool isBase = (KH_EQU(row[2], "1"));
    ...
    if (!finalSystem.empty())
    {
        ...
        if (KH_EQU(current_level, "Unknown"))
        {
    ...
    else
    {
        ...
        if (!obprox.empty())
        {
                 ...
                 itr != obprox.end(); ++itr)
            {
                ...
                std::string pis = (KH_EQU(keyc, 'A')) ? you : them;
                whodat << pis << "\n";
            }
