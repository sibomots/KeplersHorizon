...
bool RetreatCommand::invoke(void)
{
    ...
    std::string q = "SELECT at_hex, escape_pending FROM ships WHERE game_id=?"
    ...
    std::string qq = "UPDATE ships SET at_hex=?, escape_pending=0 "
    ...
    std::string qqq = "SELECT name FROM star_systems WHERE module_id=? "
                      " AND hex_id=?";
    ...
    std::string qqqq = "UPDATE combat_state AS cs "
                       " JOIN ( "
