    ...
    class Builder {
         public:
           ...
           std::unique_ptr<ICmd> build() {
                return std::unique_ptr<RepairCommand>(new RepairCommand(_ship_code, _attribute, _amount));
           }
    };
   
private:
    RepairCommand(const std::string& ship_code, const std::string& attribute, int amount) :
          m_ship_code(ship_code),
