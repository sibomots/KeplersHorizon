...
static int get_user_id_from_token(const std::string& token)
{
    ...
    auto rows = db.Query("SELECT user_id FROM sessions WHERE token=?", {token});
    if (rows.empty())
    {
        return 0;
    }
                      ...
                      HttpResponse* resp)
{
    ...
    // BUGBUG this is weaksauce.  Why is simply user_id is 0, a failure to auth?
    if (KH_EQU(user_id, 0))
    {
        resp->status = 401;
        resp->body = json_error("not authenticated");
    ...
    {
        Logger::instance().error("[ROOMMGR] joinRoom failed on code: " + code);
        Logger::instance().error("[ROOMMGR] joinRoom failed on user_id: " +
                       ...
                       HttpResponse* resp)
{
    ...
    {
        Logger::instance().error("[ROOMMGR] leaveRoom failed on code: " + code);
        Logger::instance().error("[ROOMMGR] leaveRoom failed on user_id: " +
                                std::to_string(user_id));
        resp->status = 400;
                        ...
                        HttpResponse* resp)
{
    ...
    {
        Logger::instance().error("[ROOMMGR] setModule failed on code: " + code);
        Logger::instance().error("[ROOMMGR] setModule failed on module_id: " + module_id);
        resp->status = 400;
                       ...
                       HttpResponse* resp)
{
    ...
    if (KH_EQU(game_id, 0))
    {
        Logger::instance().error("[ROOM] startGame failed on code: " + code);
        Logger::instance().error(
            "[ROOM] startGame failed on singleplayer: " + (singleplayer) ? "true"
                                                                     : "false");
        resp->status = 400;
...
{
    if (KH_EQU(req->path, KH_REST_ROOMS_ENDPT))
    {
        if (KH_EQU(req->method, "GET"))
        {
            handle_rooms_list(req, resp);
        }
        else if (KH_EQU(req->method, "POST"))
        {
    ...
    std::string code =
        (KH_EQU(slash, std::string::npos)) ? remainder : remainder.substr(0, slash);
    std::string action =
        (KH_EQU(slash, std::string::npos)) ? "" : remainder.substr(slash + 1);
