...
// BIGBUG This file uses a divergent saved_games schema (columns:
// original_game_id, room_code, state_json, round, player_a_name,
// player_b_name) that do not exist in Game.sql. The API save/load
// endpoints are broken and unused by the frontend. The authoritative
// save/load system is in save_command.cpp (in-game commands).
...
{
    DatabaseManager& db = DatabaseManager::instance();
    auto rows = db.Query("SELECT username FROM users WHERE id=?", {user_id});
    if (rows.empty())
    {
        return "";
    }
...
void handle_game_save(const HttpRequest* req, HttpResponse* resp)
{
    ...
    auto game_rows = db.Query(
        "SELECT state_json, module_id FROM games WHERE id=?", {game_id});
    if (game_rows.empty())
    {
        resp->status = 404;
    ...
    auto room_rows =
        db.Query("SELECT room_code FROM rooms WHERE game_id=?", {game_id});
    ...
    db.Exec("INSERT INTO saved_games(save_name, user_id, original_game_id, "
            ...
            "VALUES(?,?,?,?,?,?,?,?)",
            {save_name, user_id, game_id, room_code, state_json, round,
...
void handle_saves_list(const HttpRequest* req, HttpResponse* resp)
{
    ...
    DatabaseManager& db = DatabaseManager::instance();
    auto rows =
        db.Query("SELECT id, save_name, round, player_a_name, player_b_name, "
                 "saved_at FROM saved_games WHERE user_id=? ORDER BY saved_at "
                 "DESC LIMIT 20",
                 {user_id});
    ...
    for (size_t i = 0; i < rows.size(); ++i)
    {
        ...
        o << "{\"id\":" << rows[i][0] << ",\"name\":\""
          ...
          << ",\"round\":" << rows[i][2] << ",\"player_a\":\""
          << json_escape(rows[i][3]) << "\""
...
void handle_save_load(int save_id, const HttpRequest* req, HttpResponse* resp)
{
    ...
    // Get save data
    auto save_rows =
        db.Query("SELECT save_name, state_json, player_a_name, player_b_name "
...
void handle_save_delete(int save_id, const HttpRequest* req, HttpResponse* resp)
{
    ...
    // Check ownership
    auto rows = db.Query("SELECT id FROM saved_games WHERE id=? AND user_id=?",
                         {save_id, user_id});
    if (rows.empty())
    {
        resp->status = 404;
...
void handle_saves(const HttpRequest* req, HttpResponse* resp)
{
    ...
    if (KH_EQU(path, KH_REST_SAVED_ENDPT)
          && KH_EQU(req->method, "GET"))
    {
    ...
    if (KH_EQU(path, KH_REST_SAVE_ENDPT)
           && KH_EQU(req->method, "POST"))
    {
    ...
    if (KH_EQU(path.rfind(KH_REST_GAMES_PATH, 0), 0) &&
        KH_NEQ(path.find("/save"), std::string::npos))
    {
        // Ignore the ID in URL, use session instead
        if (KH_EQU(req->method, "POST"))
        {
            handle_game_save(req, resp);
        }
        else
    ...
    if (KH_EQU(path.rfind(KH_REST_SAVED_ENDPT, 0), 0))
    {
        std::string remainder = path.substr(11);
        size_t slash_pos = remainder.find('/');
        std::string id_str = (KH_EQU(slash_pos, std::string::npos))
                                 ? remainder
                                 : remainder.substr(0, slash_pos);
        std::string action = (KH_EQU(slash_pos, std::string::npos))
                                 ? ""
                                 : remainder.substr(slash_pos + 1);

        int save_id = std::atoi(id_str.c_str());

        if (KH_EQU(action, "load") && KH_EQU(req->method, "POST"))
        {
            handle_save_load(save_id, req, resp);
        }
        else if (action.empty() && KH_EQU(req->method, "DELETE"))
        {
