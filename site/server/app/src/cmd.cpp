                                  ...
                                  std::string& errmsg)
{
    ...
    const std::string parsediag = std::format(
        "[PARSER] Parse result for '{}' = {}", cmdline, parse_result);
    Logger::instance().debug(parsediag);
    ...
    if (parse_result != 0)
    {
        ...
        const std::string parseerr = std::format(
            "[PARSER] Parse error for '{}': {}", cmdline, errmsg);
        Logger::instance().error(parseerr);
    }
    return parse_result;
}
...
bool handle_usr_command(const HttpRequest* req, HttpResponse* resp)
{
    ...
    if (cmdline.empty())
    {
        Logger::instance().debug("[REST] cmdline empty");
        resp->status = 400;
    ...
    if (KH_EQU(parse_result, 0))
    {
        ...
        if (new_game_id != context.game_id)
        {
            ...
            Logger::instance().debug(
               std::format("[REST] Updated session game_id to {}",
                                    new_game_id));
        }
    ...
    else
    {
        ...
        Logger::instance().error("[REST] User command: " + cmdline);
        Logger::instance().error("[REST] Parser error: " + errmsg);
    }
    return false; // not done
}
