...
void Telemetry::add_tell(int game_id, char player, const std::string& msg)
{
    ...
    std::string q = "INSERT INTO telemetry_queue(game_id, target_player, "
                    "message) VALUES(?, ?, ?)";
...
Telemetry::QueuedMessages Telemetry::get_queued_messages(char player)
{
    ...
    std::string sent_col = (KH_EQU(player,'A')) ? "sent_to_A" : "sent_to_B";
    std::string both_q =
        "SELECT id, message FROM telemetry_queue WHERE game_id=? "
        "AND target_player='BOTH' AND " +
...
void Telemetry::mark_messages_sent(const QueuedMessages& msgs)
{
    ...
    {
        std::string sent_col = (KH_EQU(msgs.player, 'A')) ? "sent_to_A" : "sent_to_B";
        std::string id_list;
...
void Telemetry::tell(PlayerTarget target, const std::string& msg)
{
    ...
    char target_player = (KH_EQU(target, PlayerTarget::ME))
                             ? requesting_player
                             : requesting_player ^ 0x03;

    std::ostringstream oss;
    oss << "[" << requesting_player << "->" << target_player << "] "
        << msg.c_str();
    Logger::instance().user(oss.str());
...
{
    DatabaseManager& db = DatabaseManager::instance();
    char opponent = player ^ 0x03; // A or B
...
void Telemetry::status(char player, HttpResponse* resp)
{
    ...
    // BIGBUG: This is so broken.
    // BIGBUG: Why are we  using game_id as a flag for this??
    if (KH_EQU(game_id, 0))
    {
    ...
    char oppOwner = selfOwner ^ 0x03;

    // Look up actual usernames from game_seats
    std::string seat_q = "SELECT u.username FROM users u "
