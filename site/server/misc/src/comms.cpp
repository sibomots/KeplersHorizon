...
{
    bool done = false;
    if (KH_EQU(req->path, KH_REST_LOGIN_ENDPT))
    {
        handle_login(req, resp);
    }
    else if (KH_EQU(req->path, KH_REST_REGISTER_ENDPT))
    {
        handle_register(req, resp);
    }
    else if (KH_EQU(req->path, KH_REST_LOGOUT_ENDPT))
    {
        handle_logout(req, resp);
    }
    else if (KH_EQU(req->path, KH_REST_STATE_ENDPT))
    {
        handle_state(req, resp);
    }
    else if (KH_EQU(req->path, KH_REST_COMMAND_ENDPT))
    {
        done = handle_usr_command(req, resp);
    }
    else if (KH_EQU(req->path, KH_REST_EVENTS_ENDPT))
    {
        handle_events(req, resp);
    }
    else if (KH_EQU(req->path, KH_REST_MODULES_ENDPT))
    {
        handle_modules_list(req, resp);
    }
    else if (KH_EQU(req->path, KH_REST_ROOMS_ENDPT)
             || starts_with(req->path, KH_REST_ROOMS_PATH))
    {
...
std::string http_serialize(const HttpResponse* pr)
{
    ...
    oss << "HTTP/1.1 " << pr->status << " " << status_text(pr->status) << "\r\n"
...
HttpRequest* http_parse(int fd)
{
    ...
    size_t line_end = data.find("\r\n");
    if (KH_EQU(line_end, std::string::npos))
    {
        return preq;
    }
    ...
    size_t header_end = data.find("\r\n\r\n");
    if (KH_EQU(header_end, std::string::npos))
    {
    ...
    {
        size_t e = data.find("\r\n", pos);
        if (KH_EQU(e, std::string::npos) || (e > header_end))
        {
            break;
        }
...
AuthContext require_auth(const HttpRequest* req, HttpResponse* resp)
{
    ...
    if (KH_NEQ(a.game_id, 0) && KH_EQU(a.player, 0))
    {
        Logger::instance().debug("[COMMS][require_auth] User " + a.username +
