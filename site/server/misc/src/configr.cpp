...
{
    // values_ always contains the correct type for each key because
    // reset_defaults() establishes it and set_user_value<K> preserves it.
...
bool Configr::parse(int argc, char** argv) noexcept
{
    ...
    static option longopts[] = {{"version", no_argument, nullptr, 'v'},
...
// Summary report
void Configr::summary()
{
    Logger::instance().debug("Configuration summary:");
    if (get<Key::help>())
    {
        Logger::instance().debug("help requested");
    }
    if (get<Key::version>())
    {
        Logger::instance().debug("version requested");
    }
    if (get<Key::monitor>())
    {
        Logger::instance().debug("monitor requested");
    }
    if (has<Key::dbhost>())
    {
        Logger::instance().debug(std::format("dbhost: {}", get<Key::dbhost>()));
    }
    if (has<Key::dbname>())
    {
        Logger::instance().debug(std::format("dbname: {}", get<Key::dbname>()));
    }
    if (has<Key::dbusr>())
    {
        Logger::instance().debug(std::format("dbusr: {}", get<Key::dbusr>()));
    }
    if (has<Key::ai>())
    {
        Logger::instance().debug(std::format("ai: {}", get<Key::ai>()));
    }
    if (has<Key::port>())
    {
        Logger::instance().debug(std::format("port: {}", get<Key::port>()));
    }
    if (has<Key::log>())
    {
        Logger::instance().debug(std::format("log: {}", get<Key::log>()));
    }
}
