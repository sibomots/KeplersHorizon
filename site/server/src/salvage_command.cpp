...
void SalvageCommand::do_scan()
{
    ...
    auto ships = db.query(
        ...
        "' AND at_system IS NOT NULL AND at_system != '' AND destroyed_at IS NULL");
            ...
            else
            {
                ...
                if (roll < disc_chance)
                {
                    ...
                    db.exec(
                        ...
                        std::to_string(game_id) + "," + std::to_string(salv_id) +
                        ",'" + std::string(1, me) + "'," + std::to_string(s.round) + ")");
    ...
    if (total_found == 0)
    {
        out << "\nNo salvageables found. Try scanning in SYDRA or similar systems.\n";
    }
    else
    {
...
bool SalvageCommand::do_salvage()
{
    ...
    if (m_target_name.empty())
    {
        auto avail = db.query(
            ...
            "WHERE s.system_name='" + db.esc(ship.at_system) + "' "
            "AND ds.game_id=" + std::to_string(game_id) +
            " AND ds.depleted=0");
    ...
    auto target = db.query(
        "SELECT s.id, s.name, s.hazard_chance, s.hazard_damage_min, "
        "s.hazard_damage_max, s.max_salvages, ds.times_salvaged "
        "FROM salvageables s "
        "JOIN discovered_salvageables ds ON s.id=ds.salvageable_id "
        "WHERE s.system_name='" + db.esc(ship.at_system) + "' "
        "AND UPPER(s.name) LIKE UPPER('%" + db.esc(m_target_name) + "%') "
        "AND ds.game_id=" + std::to_string(game_id) +
        " AND ds.depleted=0 LIMIT 1");

    if (target.empty())
    {
        Telemetry::getInstance().write(
            "SALVAGE: No salvageable matching '" + m_target_name +
            "' found in " + ship.at_system + ".");
        return false;
    }
    ...
    int max_salvages = target[0][5].empty() ? -1 : std::atoi(target[0][5].c_str());
    ...
    {
        Telemetry::getInstance().write(
            "SALVAGE: " + ship.name + " needs Drones (DR) to salvage safely.");
        return false;
    }
    ...
    auto drops = db.query(
        ...
        "FROM salvageable_drops WHERE salvageable_id=" + std::to_string(salv_id));
    ...
    for (const auto& drop : drops)
    {
        ...
        if (item_type == "resource")
        {
            ...
            if (item_name == "RARE_EARTH") col = "cargo_rare_earth";
            else if (item_name == "RADIOACTIVE") col = "cargo_radioactive";
            else if (item_name == "CRYSTALLINE") col = "cargo_crystalline";
            else if (item_name == "VOLATILE") col = "cargo_volatile";
            else if (item_name == "WATER") col = "cargo_water";
            else if (item_name == "ORGANIC") col = "cargo_organic";
            else if (item_name == "EXOTIC") col = "cargo_exotic";

            db.exec("UPDATE ships SET " + col + "=" + col + "+" +
                    std::to_string(qty) + " WHERE game_id=" +
                    std::to_string(game_id) + " AND owner='" + std::string(1, me) +
        ...
        {
            db.exec("UPDATE ships SET missiles=missiles+" + std::to_string(qty) +
                    " WHERE game_id=" + std::to_string(game_id) + " AND owner='" +
                    std::string(1, me) + "' AND ship_code='" + db.esc(m_ship_code) + "'");
        ...
        else if (item_type == "credits")
        {
            ...
            db.exec("UPDATE games SET " + cred_col + "=" + cred_col + "+" +
                    std::to_string(qty) + " WHERE id=" + std::to_string(game_id));
    ...
    {
        std::string header = "SALVAGE: " + ship.name + " salvaged " + salv_name + ":\n";
        result.str(header + result.str());
    }

    // Update salvage count and check depletion
    db.exec("UPDATE discovered_salvageables SET times_salvaged=times_salvaged+1 "
            "WHERE game_id=" + std::to_string(game_id) +
            " AND salvageable_id=" + std::to_string(salv_id));
