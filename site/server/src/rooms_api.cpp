...
void handle_room_get(const std::string& code, const HttpRequest* req, HttpResponse* resp)
{
    RoomManager& rm = RoomManager::getInstance();
    RoomInfo room = rm.getRoom(code);
...
void handle_room_join(const std::string& code, const HttpRequest* req, HttpResponse* resp)
{
    std::string token = pick_bearer(req);
    int user_id = get_user_id_from_token(token);
    if (user_id == 0) {
...
void handle_room_leave(const std::string& code, const HttpRequest* req, HttpResponse* resp)
{
    std::string token = pick_bearer(req);
    int user_id = get_user_id_from_token(token);
    if (user_id == 0) {
...
void handle_room_scenario(const std::string& code, const HttpRequest* req, HttpResponse* resp)
{
    std::string token = pick_bearer(req);
    int user_id = get_user_id_from_token(token);
    if (user_id == 0) {
...
void handle_room_start(const std::string& code, const HttpRequest* req, HttpResponse* resp)
{
    std::string token = pick_bearer(req);
    int user_id = get_user_id_from_token(token);
    if (user_id == 0) {
...
void handle_rooms(const HttpRequest* req, HttpResponse* resp)
{
    ...
    // Extract room code from path: /api/rooms/{code} or /api/rooms/{code}/action
    ...
    std::string code = (slash == std::string::npos) ? remainder : remainder.substr(0, slash);
    std::string action = (slash == std::string::npos) ? "" : remainder.substr(slash + 1);
