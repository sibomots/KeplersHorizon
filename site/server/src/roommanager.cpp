...
std::string RoomManager::createRoom(int user_id, const std::string& name)
{
    ...
    std::string sql = "INSERT INTO rooms(room_code, name, created_by, seat_a, status) VALUES('" +
                      db.esc(code) + "','" + db.esc(name) + "'," + 
                      std::to_string(user_id) + "," + std::to_string(user_id) + ",'waiting')";
    db.exec(sql);
...
bool RoomManager::joinRoom(const std::string& code, int user_id)
{
    ...
    auto rows = db.query("SELECT id, seat_a, seat_b, status FROM rooms WHERE room_code='" +
                         db.esc(code) + "'");
    ...
    if (seat_a == user_id || seat_b == user_id) return false;
    ...
    if (!check.empty() && !check[0][0].empty() && !check[0][1].empty()) {
        db.exec("UPDATE rooms SET status='ready' WHERE room_code='" + db.esc(code) + "'");
    }
...
bool RoomManager::deleteRoom(const std::string& code, int user_id)
{
    ...
    auto rows = db.query("SELECT id, created_by, status FROM rooms WHERE room_code='" +
                         db.esc(code) + "'");
    if (rows.empty()) return false;
    ...
    if (created_by != user_id) return false;
    if (status == "playing") return false;
...
std::vector<RoomInfo> RoomManager::listOpenRooms()
{
    ...
    auto rows = db.query(
        "SELECT r.id, r.room_code, r.name, r.created_by, u.username, "
...
RoomInfo RoomManager::getRoomByUser(int user_id)
{
    ...
    auto rows = db.query(
        ...
        " OR seat_b=" + std::to_string(user_id) + ") AND status != 'finished' LIMIT 1");
...
{
    DatabaseManager& db = DatabaseManager::getInstance();
    auto rows = db.query("SELECT id FROM rooms WHERE room_code='" + db.esc(code) + "'");
    return !rows.empty();
}

bool RoomManager::setScenario(const std::string& code, const std::string& scenario)
{
    DatabaseManager& db = DatabaseManager::getInstance();
...
int RoomManager::startGame(const std::string& code)
{
    ...
    if (room.id == 0) return 0;
    if (room.status != "ready") return 0;
    if (room.seat_a == 0 || room.seat_b == 0) return 0;
    ...
    db.exec("INSERT INTO game_seats(game_id, user_id, seat) VALUES(" +
            std::to_string(game_id) + "," + std::to_string(room.seat_a) + ",'A')");
    db.exec("INSERT INTO game_seats(game_id, user_id, seat) VALUES(" +
