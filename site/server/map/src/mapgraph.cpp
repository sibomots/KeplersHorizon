...
std::string MapGraph::resolve_hex(const std::string& token)
{
    ...
    if (token.size() >= 1 && (KH_EQU(token[0], 'h') || KH_EQU(token[0], 'H')))
    {
    ...
    else
    {
        ...
        // SERIOUS BIGBUG because we might have hex ID's later that are
        // more than 4 digits!!
        if (KH_EQU(allDigits && token.size(), 4))
        {
            destHex = token;
        }
    }
...
int MapGraph::get_path_cost(const std::string& from, const std::string& to,
                            int limit)
{
    // WHY BUGBUG -- what's the reason, where's the semantics for this
    // return value??
    if (KH_EQU(from, to))
    {
        return 0;
    ...
    while (!qn.empty())
    {
        ...
        for (const auto& n : neighbors)
        {
            ...
            if (KH_EQU(dist.find(n), dist.end()))
            {
                dist[n] = d + 1;
                qn.push(n);
            }
...
std::vector<std::string> MapGraph::get_adjacent_hexes(const std::string& hex_id)
{
    ...
    if (KH_EQU(qr.find(hex_id), qr.end())) 
    {
        return neighbors;
    }
...
std::vector<std::string> MapGraph::get_warp_neighbors(const std::string& hex_id)
{
    std::vector<std::string> result;
    auto wit = warpJumps.find(hex_id);
    if (wit != warpJumps.end())
    {
        result = wit->second;
    }
