...
function xxxappendLine(text, cls) {
  const log = $("consoleLog");
  if (!log) return;
...
///////////////////////////////////////////////////////////////////////////////////
  ...
  function renderStatus() {
    ...
    setText("stVP", st?.vp ? (S.playerNames.A + ":" + st.vp.A + "  " + S.playerNames.B + ":" + st.vp.B) : "-");
    ...
    if (elTitle) {
      ...
      elTitle.textContent = "Console for " + playerName;
    }
  ...
  async function apiFetchState() {
    ...
    // If server reports no active game, redirect to lobby
    if (j.state && j.state.gameId === 0) {
  ...
  async function apiCommand(cmd) {
    ...
    if (j && typeof j.event === "string" && j.event.length > 0) {
      // Split on newlines (JSON.parse already unescsapes \n to actual newlines)
